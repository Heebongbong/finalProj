...
function boardAddList(){
	...
	let keyword = $('#market_keyword').val();
	let category = $('#market_category').val();
	if($('#market_keyword').val()!=null){
		keyword = replace_keyword(keyword);
	}
	if($('#market_category').val()!=null){
		category = replace_keyword(category);
	}
				...
				"<div class='board_user_wrap'>" +
					...
					"<div class='detail_modal_overlay'>" +
						"<div class='detail_modal_window'>"+
							"<a href='javascript:fraud_check_window()'>더치트 검색</a>"+
                ...
                "<div class='board_main_cont'>" +
					...
					"<div class='board_title_price'>" +
						"<div class='board_main_title'>" +
							board.title +
						"</div>" +
						"<div class='board_main_price'>" +
							board.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ',') + "원" +
						"</div>" +
					"</div>" +
